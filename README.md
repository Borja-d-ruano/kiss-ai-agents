...
**Design claim:** replace stacks like LangChain for orchestration with **under ~1000 lines** of executable code (Python + shell), while your **business logic stays in `.md` and data files**, not in the framework.
...
The runtime is a **mail carrier**: it does not implement your CRM rules; it **loads**, **calls the model**, and **writes files**.
...
Follow your organization’s policy for this repo; keep **secrets out of Markdown** (use environment variables). Chat session files under `**/input/session/` are typically **gitignored** — do not commit customer transcripts.
